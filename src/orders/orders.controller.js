...
function bodyHasDeliverTo(req, res, next) {
    ...
    // No need to set res.locals.reqBody 
    return next();
}
...
function bodyHasDishQuantity(req, res, next) {
    ...
    const indexesOfDishesWithoutQuantity = dishes.reduce(
        (acc, dish, index) => {
...
function update(req, res, next) {
    const { orderId } = req.params; 
    const order = res.locals.order; 
    const { data: { deliverTo, mobileNumber, dishes, quantity, id, status } = {} } = req.body; // Destructure request body
